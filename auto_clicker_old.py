...
class AutoClicker:
    ...
    @staticmethod
    def click_pyautogui(button):
        """Faz um clique usando pyautogui"""
        if button == Button.left:
    ...
    def __init__(self, cps=20, enable_right_click=False, hotkey='<ctrl>+<shift>+a', status_callback=None):
        ...
        # Controle baseado em eventos reais do usuário
        self.user_released_button = False
        self.last_user_press_time = 0
    ...
    def perform_clicks(self, button):
        ...
        # Reseta a flag de release do usuário
        self.user_released_button = False
        self.last_user_press_time = time.time()

        # Loop de cliques
        while self.clicking and self.auto_click_enabled:
            # Verifica se usuário realmente soltou o botão
            # (não teve eventos de PRESS recentes)
            time_since_last_press = time.time() - self.last_user_press_time

            if self.user_released_button and time_since_last_press > 0.15:
                print(f"[DEBUG] Usuário SOLTOU o botão confirmado - parando (após {click_count} cliques)")
                break

            # Faz o clique
            self.click_pyautogui(button)
            click_count += 1
    ...
    def on_click(self, x, y, button, pressed):
        ...
        # Durante auto-click, processa eventos para detectar quando usuário solta
        if self.clicking and self.current_auto_button == button:
            if pressed:
                # Evento de PRESS - usuário ainda está segurando (ou clique automático)
                # Atualiza timestamp apenas se for provável evento do usuário
                current_time = time.time()
                if current_time - self.last_user_press_time > 0.04:  # Pelo menos 40ms desde último
                    print(f"[DEBUG] PRESS detectado - usuário ainda segurando")
                    self.last_user_press_time = current_time
                    self.user_released_button = False  # Cancela qualquer release anterior
            else:
                # Evento de RELEASE - usuário pode ter soltado
                print(f"[DEBUG] RELEASE detectado - possível soltar")
                self.user_released_button = True
            return

        # Botão esquerdo - iniciar cliques automáticos (se modo ativado)
        if button == Button.left and self.auto_click_enabled and pressed and not self.clicking:
            print(f"[DEBUG] Botão esquerdo PRESSIONADO (usuário) - iniciando")
        ...
        if button == Button.right and self.enable_right_click and self.auto_click_enabled and pressed and not self.clicking:
            print(f"[DEBUG] Botão direito PRESSIONADO (usuário) - iniciando")
